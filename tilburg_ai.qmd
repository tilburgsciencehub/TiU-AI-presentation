...
- Algorithmic course allocation (students &rarr; courses)
- Personalized studying experience and modular programs
- Novel budget allocation mechanisms
- Automated, real-time scheduling (predicted attendance)
- AI-facilitated blended learning (summarized content)
- Open development with faculty and students
...
- Researcher from TiSEM (but: talked to 10+ faculty and staff members, across schools and divisions)
...
#
![<font color=grey> Examplary use of ChatGPT: Prompt, ask & refine</font>](img/chatgpt.png)

<!-- I need this example to be swapped to adhere to prompt, ask & refine. Plus an educational example, e.g., generating exam questions.-->
...
- Content delivery
  ...
  - Improving slides, generating subtitles
  
- Automation
  - Semi-automated grading 
  - "Patchworking" different systems
...
- Editing and writing coach (e.g., finding good titles & writing abstracts, feedback on writing style, developing paragraph structures, synthesizing)
- Ideation and problem solving (e.g., "which `R` package to use for X?") 
- Fixing or annotating computer code (e.g., "I have a bug in this code - do you spot it?")
- A "virtual" teacher, 24/7, immediate response (e.g., "why is X important?", "can you explain how X works?")
- Preparing for the labor market (e.g., redesigning resumes)
- Realizing efficiencies (e.g., converting documents, taking over part of one's job)

## How do we use it in research? {.smaller}
...
- Ideation (e.g., generating outlines)
- Research assistant (e.g., generating stimuli, summarizing articles)

# 3. What are new ways <br>in which we can use AI?{background-color="#036"}
...
- ChatGPT is "just" an interface to OpenAI's APIs (and its latest version, ["GPT-4 API"](https://openai.c
  om/product/gpt-4), was just announced yesterday)
    - creativity, advanced reasoning (bar exam: 90th percentile, biology: 99th)
    - already used in education: student feedback (Khan Academy), conversations in foreign languages (Duolingo)

::: goal
In __rolling out AI in Tilburg__, let's think about
(1) applying AI __manually__ ("hey, here's an exam question - can you create 5 different versions?"), and 
(2) automating the use of AI in scaling up successful use cases.
:::

## Future use cases for teaching {.smaller}
...
- Low-hanging fruits
  ...
  - Improving assessment (e.g., more formative testing, generation of (open) questions)
- Challenging but possible with today's technology
  - Automating assessment (e.g., written feedback, automated grading)
  - Personal chat bot coach (e.g., study coach, career coach)
- Still some investment needed (but we can do it!)
...
## Risks of AI for Tilburg University {.smaller}

:::: {.columns}

::: {.column width="50%"}
__Risks of NOT embracing AI__

- Losing students to other universities?
- Falling behind in research productivity?
- Not fulfilling our ambitious strategy (e.g., blended learning)
- Lack of modernization of infrastructure and inability to automate
- Possibility of students and staff using AI illegally
- Not realizing efficiency gains (e.g., automation)

:::

::: {.column width="50%" .fragment}

__Risks when embracing AI__

- Copying and cheating (but: on-campus exams, "citability" of ChatGPT)
- "Students do not learn X" (but: more feedback, more complicated problems)
- Quality control when scaling up (but: firms can do it, already!)
- Teaching delivery (but: also Powerpoint and Covid changed teaching)
- Staffing (but: we can redefine roles) <!-- , focus on new content and student coaching-->
...
- (1) Educate
  - staff and faculty (no-coding, coding, algorithmic) 
- (2) Encourage 
  - build an active community to exchange experiences and meet up
  - promote use among staff, faculty and students (e.g., campus license for ChatGPT!)
- (3) Experiment
  - gather empirical evidence on what works well (and what not)
  - open up programmatic access to TiU infrastructure for app development (e.g., own data, common data; build mashups)
...
## Conclusion {.smaller}
- Our faculty already explores the use of AI in various aspects of education and research -- __how does Tilburg Unviersity facilitate them?__
